...
def config() -> argparse.Namespace:
    ...
    # search config
    parser.add_argument("--max_depth", type=int, default=2, help="Max depth for search agents.")
    parser.add_argument("--branching_factor", type=int, default=5, help="Branching factor at each step for the search agent.")
    parser.add_argument("--search_algo", type=str, default="mcts", help="Search algorithm to use", choices=["mcts", "vf", "bfs", "dfs"])
    ...
    parser.add_argument('--mcts_iterations', type=int, default=5, help='Number of iterations for MCTS')
    parser.add_argument('--mcts_exploration', type=float, default=1.7, help='Exploration parameter for MCTS')
        ...
        try:
            ...
            while True:
                ...
                if args.agent_type == "search" and type(action) == list:
                    ...
                    if args.search_algo == "mcts":
                        print("STARTING MCTS")
                        mcts = MCTS(
                            root_state=trajectory,
                            agent=agent,
                            intent=intent,
                            images=images,
                            meta_data=meta_data,
                        ...
                        best_action, best_score = mcts.search()
                        best_actions = [best_action]
                        all_candidates.append(f"MCTS selected action: {best_action['raw_prediction']} (score: {best_score})")
                    elif args.search_algo in ["vf", "bfs", "dfs"]:
                        ...
                        raise ValueError(f"Unknown search algorithm: {args.search_algo}")
